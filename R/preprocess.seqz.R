...
preprocess.seqz <- function(seg, ploidy0 = NULL, chr.in.names = TRUE, outputdir = NULL) {
  ...
  run_name <- gsub("[._]?small\\.seqz\\.gz|[._]?seqz\\.gz|\\.gz", "", basename(seg))
  if (chr.in.names) {
    extract <- sequenza.extract(seg, chromosome.list = paste("chr", c(1:22, "X", "Y"), sep = ""), gamma = 60, kmin = 50)
  } else {
  ...
  write.table(t(info_seg), paste0(outputdir, "/", run_name, ".info.seg.txt"), sep = "\t", row.names = F, quote = FALSE)
  allele.cn <- sequenza:::baf.bayes(
    Bf = seg.tab$Bf, CNt.max = 20, depth.ratio = seg.tab$depth.ratio, avg.depth.ratio = 1,
  ...
  # Making imput file
  seg <- data.frame(
    SampleID = as.character(run_name), Chromosome = seg.tab$chromosome, Start_position = seg.tab$start.pos,
