...
scar_score <- function(seg, reference = "grch38", chr.in.names = TRUE, seqz = FALSE, ploidy = NULL, sizelimitLOH = 15e6, outputdir = NULL) {
  ...
  if (seqz == TRUE) {
    ...
    seg <- preprocess.seqz(seg, ploidy0 = ploidy, chr.in.names = chr.in.names, outputdir = outputdir)
    cat("Preprocessing finished \n")
  } else {
    seg <- data.table::fread(seg, header = T, check.names = F, stringsAsFactors = F, sep = "\t")
    seg[, 9] <- seg[, 8]
    seg[, 8] <- seg[, 7]
    seg[, 7] <- seg[, 6]
